...
void CShape::SetShader(mat4 &mxModelView, mat4 &mxProjection, GLuint uiShaderHandle)
{
	...
	m_uiAmbient = glGetUniformLocation( m_uiProgram, "SpotDir" );
	glUniform4fv(m_uiAmbient, 1, m_AmbientProduct); 

	m_uiSpotDir = glGetUniformLocation(m_uiProgram, "AmbientProduct");
	glUniform4fv(m_uiSpotDir, 1, m_SpotDir);
...
void CShape::SetModelViewMatrix(mat4 &mat)
{
	m_mxModelView = mat;
	m_bMVUpdated = true;
}
...
vec4 CShape::PhongReflectionModel(vec4 vPoint, vec3 vNormal, vec4 vLightPos, vec4 vLightI)
{
...
GLuint CShape::LoadPngImageAndSetTextureObject(const char *texMapName)
{
...
GLubyte * CShape::LoadPngImageSOIL(const char *file_name, int &outWidth, int &outHeight, int &outBitDepth, int &outFormat)
{
...
// 
GLubyte * CShape::LoadPngImageLIBPNG(const char *file_name, int &outWidth, int &outHeight, int &outBitDepth, int &outFormat) {
